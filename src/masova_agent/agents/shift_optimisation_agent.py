...
SCHEDULABLE_ROLES = {"STAFF", "DRIVER"}
...
async def _get_weekly_forecast(
    ...
    """
    Returns forecast keyed by day-of-week (0=Mon) → predicted quantity for that day.
    The platform's demand-forecast response has daily (not hourly) granularity,
    so per-slot demand is derived by splitting each day's total across shift slots.
    Falls back to empty dict if unavailable.
    """
    forecast_headers = {**headers, "X-Selected-Store-Id": store_id}
    res = await client.get(
        f"{backend_url}/api/bi?type=demand-forecast&period=WEEKLY",
    ...
    for item in raw.get("itemForecasts", []):
        for day in item.get("dailyBreakdown", []):
            date_str = day.get("date")
            qty = day.get("quantity", 0)
            if not date_str:
                continue
            dow = datetime.fromisoformat(date_str).weekday()
            forecast[dow] = forecast.get(dow, 0) + qty
...
def _build_draft_shifts(
    ...
    for day_offset in range(7):
        ...
        dow = day.weekday()
        # Forecast is daily, not hourly — split evenly across the day's shift slots
        day_total_demand = forecast.get(dow, 0)
        slot_share = day_total_demand / len(SHIFT_SLOTS) if day_total_demand else 0

        for slot in SHIFT_SLOTS:
            slot_demand = slot_share
            ...
            for _ in range(staff_count):
                ...
                draft_shifts.append({
                    ...
                    "type": "REGULAR",
                    "scheduledStart": shift_start.isoformat(),
                    "scheduledEnd": shift_end.isoformat(),
                    "status": "PENDING_APPROVAL",
                    "isMandatory": False,
                    "notes": f"Auto-drafted by Shift Optimisation Agent — {slot['name']} (demand: {slot_demand:.0f})",
                })
