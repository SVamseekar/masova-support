        ...
        for store in stores:
            ...
            for supplier_id, items in supplier_items.items():
                po_payload = {
                    ...
                    "items": [_clamped_po_line(item) for item in items],
                    "notes": (
                        "Auto-generated by Inventory Reorder Agent at "
                ...
                if po_res.status_code in (200, 201):
                    ...
                    await _notify_manager(
                        ...
                        headers,
                        store_id,
                        f"Inventory Alert: {item_names}{more} need reordering. "
                        "Draft PO created — please review.",
                    )

    logger.info(
        "Inventory reorder complete: %d POs drafted, %d items checked", pos_drafted, items_checked
    )
    return {
...
def _clamped_po_line(item: Dict) -> Dict:
    from ..runtime.ops_contract import clamp_po_quantity

    reorder = float(item.get("reorderQuantity") or 10)
    requested = float(item.get("quantity") or reorder)
    return {
