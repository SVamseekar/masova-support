    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        for store in stores:
            ...
            campaign_payload = {
                ...
                "createdBy": "churn_prevention_agent",
            }
...
async def _find_churned_customers(
    ...
    churn_cutoff = datetime.now(timezone.utc) - timedelta(days=CHURN_WINDOW_DAYS)

    churned = []
    for customer in all_customers:
        last_order = customer.get("lastOrderDate")
        total_orders = customer.get("orderStats", {}).get("totalOrders", 0)
