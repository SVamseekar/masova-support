...
class TestDemandForecastingAgent:
    ...
    async def test_successful_forecast_returns_summary(self):
        ...
        with (
            patch("masova_agent.utils.config.get_config", return_value=_mock_config()),
            patch(
                "masova_agent.agents.demand_forecasting_agent.httpx.AsyncClient",
...
class TestInventoryReorderAgent:
    ...
    async def test_drafts_pos_for_low_stock_items(self):
        ...
        with (
            patch("masova_agent.utils.config.get_config", return_value=_mock_config()),
            patch(
                "masova_agent.agents.inventory_reorder_agent.httpx.AsyncClient",
                return_value=_async_client_ctx(client),
            ),
        ):
...
class TestChurnPreventionAgent:
    ...
    async def test_creates_campaign_for_churned_customers(self):
        ...
        churned = [{"id": "cust-1", "lastOrderDate": "2026-01-01T00:00:00Z", "totalOrders": 5}]
        ...
        with (
            patch("masova_agent.utils.config.get_config", return_value=_mock_config()),
            patch(
                "masova_agent.agents.churn_prevention_agent.httpx.AsyncClient",
                return_value=_async_client_ctx(client),
            ),
        ):
...
class TestReviewResponseAgent:
    ...
    async def test_low_rating_generates_draft(self):
        ...
        with (
            patch("masova_agent.utils.config.get_config", return_value=_mock_config()),
            patch(
                "masova_agent.agents.review_response_agent.httpx.AsyncClient",
                return_value=_async_client_ctx(client),
            ),
...
class TestShiftOptimisationAgent:
    ...
    async def test_drafts_shifts_for_store(self):
        ...
        staff = [{"id": "emp-1", "type": "KITCHEN_STAFF"}, {"id": "emp-2", "type": "CASHIER"}]
        ...
        with (
            patch("masova_agent.utils.config.get_config", return_value=_mock_config()),
            patch(
                "masova_agent.agents.shift_optimisation_agent.httpx.AsyncClient",
                return_value=_async_client_ctx(client),
            ),
        ):
...
class TestKitchenCoachAgent:
    ...
    async def test_sends_brief_to_managers(self):
        ...
        with (
            patch("masova_agent.utils.config.get_config", return_value=_mock_config()),
            patch(
                "masova_agent.agents.kitchen_coach_agent.httpx.AsyncClient",
                return_value=_async_client_ctx(client),
            ),
        ):
...
class TestDynamicPricingAgent:
    ...
    async def test_overload_triggers_price_increase_suggestion(self):
        ...
        with (
            patch("masova_agent.utils.config.get_config", return_value=_mock_config()),
            patch(
                "masova_agent.agents.dynamic_pricing_agent.httpx.AsyncClient",
                return_value=_async_client_ctx(client),
            ),
    ...
    def test_overload_message_format(self):
        ...
        msg = _overload_message("Test Store", 18, [{"name": "Biryani"}, {"name": "Pizza"}], 12)
